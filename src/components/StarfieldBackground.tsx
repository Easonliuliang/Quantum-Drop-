...
type AnimationState = 'idle' | 'transferring' | 'completed' | 'error' | 'collapsing' | 'exploding';
...
class Particle {
  ...
  constructor(w: number, h: number) {
    ...
    if (colorRand > 0.7) {
      ...
      this.color = `rgba(230, 240, 255, `;
    } else {
      this.color = `rgba(255, 255, 255, `;
    }
  }

  update(width: number, height: number, state: AnimationState, time: number) {
    if (state === 'idle') {
  ...
  draw(ctx: CanvasRenderingContext2D, state: AnimationState, width: number, height: number) {
    ctx.beginPath();
...
export const QuantumBackground: React.FC<QuantumBackgroundProps> = ({ transferState = 'idle' }) => {
  ...
  const stateRef = useRef<AnimationState>(transferState);
